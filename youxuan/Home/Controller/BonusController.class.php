...
class BonusController extends Controller {
    function index()
   {
       ...
       $todayticheng=M('order')
           ...
           ->where('o.ostatus>0 and o.osid='.$shopinfo['did'].' and to_days(ordertime) = to_days(now())')
           ->select();//所有信息
       $totaltoday=array();
       ...
       $weekticheng=M('order')
           ...
           ->join("yx_goods g on o.ogid=g.gid")
           ->field("o.buynum,o.ordertime,o.oaddtime,o.ogid,o.osid,g.gtitle,g.gticheng")//需要显示的字段
           ->where('o.ostatus>0 and o.osid='.$shopinfo['did']." and YEARWEEK(date_format(ordertime,'%Y-%m-%d')) = YEARWEEK(now())")
           ->select();//所有信息
       $totalweek=array();
       ...
       $monthticheng=M('order')
           ...
           ->join("yx_goods g on o.ogid=g.gid")
           ->field("o.buynum,o.ordertime,o.oaddtime,o.ogid,o.osid,g.gtitle,g.gticheng")//需要显示的字段
           ->where('o.ostatus>0 and o.osid='.$shopinfo['did']." and DATE_FORMAT( ordertime, '%Y%m' ) = DATE_FORMAT( CURDATE( ) , '%Y%m' )")
           ->select();//所有信息
       $totalmonth=array();
       ...
       $allticheng=M('order')
           ...
           ->join("yx_goods g on o.ogid=g.gid")
           ->field("o.buynum,o.ordertime,o.oaddtime,o.ogid,o.osid,g.gtitle,g.gticheng")//需要显示的字段
           ->where('o.ostatus>0 and o.osid='.$shopinfo['did'])
           ->select();//所有信息
       $totalall=array();
